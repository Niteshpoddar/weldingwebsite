...
async function initializeSampleData() {
  ...
  if (!db) return; // Use in-memory data if no DB connection
  ...
  getById: async (id) => {
    try {
      ...
      if (db) {
        ...
        // Fallback to in-memory storage
        return inMemoryJobs.find(job => job._id === id.toString());
      }
    } catch (error) {
  ...
  create: async (jobData) => {
    try {
      ...
      if (db) {
        ...
        // Fallback to in-memory storage
        const newId = (Math.max(...inMemoryJobs.map(j => parseInt(j._id)), 0) + 1).toString();
        const jobWithId = { ...newJob, _id: newId };
        inMemoryJobs.push(jobWithId);
        return jobWithId;
      }
    } catch (error) {
  ...
  update: async (id, jobData) => {
    try {
      ...
      if (db) {
        ...
        // Fallback to in-memory storage
        const index = inMemoryJobs.findIndex(job => job._id === id.toString());
        if (index !== -1) {
          inMemoryJobs[index] = { ...inMemoryJobs[index], ...updatedJob };
  ...
  delete: async (id) => {
    try {
      ...
      if (db) {
        ...
        inMemoryJobs = inMemoryJobs.filter(job => job._id !== id.toString());
  ...
  getAll: async () => {
    try {
      ...
      if (db) {
        ...
        // Fallback to in-memory storage
        return inMemoryTrainings;
      }
    } catch (error) {
      console.error('Error fetching trainings:', error);
      return inMemoryTrainings; // Fallback
    }
  },
  ...
  getById: async (id) => {
    try {
      ...
      if (db) {
        ...
        // Fallback to in-memory storage
        return inMemoryTrainings.find(training => training._id === id.toString());
      }
    } catch (error) {
  ...
  create: async (trainingData) => {
    try {
      ...
      if (db) {
        ...
        // Fallback to in-memory storage
        const newId = (Math.max(...inMemoryTrainings.map(t => parseInt(t._id)), 0) + 1).toString();
        const trainingWithId = { ...newTraining, _id: newId };
        inMemoryTrainings.push(trainingWithId);
  ...
  update: async (id, trainingData) => {
    try {
      ...
      if (db) {
        ...
        // Fallback to in-memory storage
        const index = inMemoryTrainings.findIndex(training => training._id === id.toString());
        if (index !== -1) {
          inMemoryTrainings[index] = { ...inMemoryTrainings[index], ...updatedTraining };
          return inMemoryTrainings[index];
        }
        return null;
      }
    } catch (error) {
  ...
  delete: async (id) => {
    try {
      ...
      if (db) {
        ...
        // Fallback to in-memory storage
        const initialLength = inMemoryTrainings.length;
        inMemoryTrainings = inMemoryTrainings.filter(training => training._id !== id.toString());
        return inMemoryTrainings.length < initialLength;
      }
    } catch (error) {
