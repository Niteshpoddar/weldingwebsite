...
export default function TrainingApplications() {
    ...
    const fetchApplications = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setApplications(data.trainingApplications || []);
        }
      } catch (error) {
  ...
  const deleteApplication = async (id) => {
    if (window.confirm('Are you sure you want to delete this training application? This action cannot be undone.')) {
      try {
        const response = await fetch(`/api/admin/applications`, {
          method: 'DELETE',
          headers: {
  ...
  const deleteResume = async (cloudinaryId) => {
    try {
  ...
  const sendCustomEmail = async (application) => {
    ...
    if (emailType === 'acceptance') {
      subject = `Congratulations! Your Training Application for ${application.trainingCourse} has been Accepted`;
      body = `Dear ${application.fullName},

We are excited to inform you that your application for ${application.trainingCourse} training has been accepted!

Our training team will contact you soon with course details, schedule, and any prerequisites you may need.
...
The Training Team`;
      ...
      subject = `Update on Your Training Application for ${application.trainingCourse}`;
      body = `Dear ${application.fullName},

Thank you for your interest in our ${application.trainingCourse} training program.

After reviewing your application, we regret to inform you that we are unable to accommodate your request at this time.

We encourage you to check our website for future training opportunities.
...
The Training Team`;
    ...
    return (
      <div className="min-h-screen section-bg flex items-center justify-center">
        <div className="text-center animate-fade-in">
          ...
          <p className="mt-4 text-primary-700">Loading training applications...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="pt-20 min-h-screen section-bg">
      {/* Header */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6 animate-fade-in">
            <div className="flex items-center animate-slide-up" style={{ animationDelay: '200ms' }}>
              ...
              <div>
                <h1 className="text-3xl font-bold text-primary-900">Training Applications</h1>
                <p className="text-primary-700">Review and manage training applications</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {applications.length === 0 ? (
          <div className="text-center py-12 animate-fade-in">
            ...
            <p className="text-primary-700 animate-slide-up" style={{ animationDelay: '400ms' }}>
              Training applications will appear here when candidates apply.
            </p>
          </div>
        ) : (
                      ...
                      <div className="flex flex-col gap-4">
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-primary-700 font-medium">Course:</span>
                            <span className="text-primary-600">{app.trainingCourse}</span>
                          </div>
                          
                          <div className="flex items-center gap-2">
                            <CalendarIcon className="h-4 w-4 text-primary-400" />
                            <span className="text-primary-700 font-medium">Applied:</span>
                            <span className="text-primary-600">{formatDateIndian(app.appliedAt)}</span>
                          </div>
